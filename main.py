...
from services.pending_actions import process_pending_actions
...
def setup_scheduler(bot: Bot) -> AsyncIOScheduler:
    ...
    Job‌ها:
      • check_expired_subscriptions   — هر روز ساعت ۰۸:۰۰ UTC
      • cleanup_stale_payments        — هر ساعت یک‌بار
      • send_daily_backups            — هر روز ساعت ۰۲:۰۰ UTC
    """
    scheduler = AsyncIOScheduler(timezone="UTC")

    # بررسی روزانه انقضای اشتراک‌ها ← هر روز ۰۸:۰۰ UTC
    scheduler.add_job(
        check_expired_subscriptions,
        trigger=CronTrigger(hour=8, minute=0),
        args=[bot],
        id="daily_expiry_check",
        name="بررسی روزانه انقضای اشتراک‌ها",
    ...
    # پردازش pending_actions — تأیید پرداخت از وب‌پنل ← هر ۱۵ ثانیه
    # وقتی ادمین از وب‌پنل پرداخت کارت به کارت را تأیید می‌کند،
    # پنل یک ردیف در pending_actions می‌نویسد. این job آن را می‌خواند
    # و اشتراک VPN را می‌سازد + به کاربر ارسال می‌کند.
    scheduler.add_job(
        process_pending_actions,
        trigger="interval",
        seconds=15,
        args=[bot],
        id="pending_actions_processor",
        name="پردازش تأییدیه‌های وب‌پنل",
        replace_existing=True,
        max_instances=1,
...
async def main() -> None:
    ...
    dp.include_router(uuid_router)         # دریافت اشتراک با UUID
