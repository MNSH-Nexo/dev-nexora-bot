...
REFERRAL_REWARD_DAYS_KEY = "referral_reward_days"


async def get_reward_days() -> int:
    """تعداد روزهای پاداش referral را از DB می‌خواند (پیش‌فرض: 3)."""
    try:
        from database import AsyncSessionLocal
        from database.crud import get_setting
        async with AsyncSessionLocal() as s:
            val = await get_setting(s, REFERRAL_REWARD_DAYS_KEY, str(DEFAULT_REWARD_DAYS))
            return int(val) if val.isdigit() and int(val) > 0 else DEFAULT_REWARD_DAYS
    except Exception:
        return DEFAULT_REWARD_DAYS
...
async def process_referral(
    ...
    # reward_days را از DB بخوان (ادمین می‌تواند از وب پنل تغییر دهد)
    reward_days = await get_reward_days()
