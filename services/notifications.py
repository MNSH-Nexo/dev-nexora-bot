...
def _expiry_warning_text(sub: Subscription, days_left: int) -> str:
    """متن هشدار انقضای اشتراک بر اساس روزهای باقی‌مانده."""
    ...
    return (
        f"{urgency} <b>هشدار انقضای اشتراک</b>\n"
        "━━━━━━━━━━━━━━━\n"
        f"اشتراک شما <b>{time_str}</b> منقضی می‌شود.\n\n"
        f"📧 ایمیل: <code>{sub.email}</code>\n"
        f"📅 تاریخ انقضا: <code>{expire_str}</code>\n\n"
        f"{note}\n\n"
        "برای تمدید از منو <b>🛒 خرید کانفیگ</b> را انتخاب کنید."
    )


def _traffic_warning_text(sub: Subscription, used_pct: float) -> str:
    """متن هشدار ترافیک."""
    used_gb = sub.used_traffic_bytes / 1024 ** 3
    limit_gb = sub.traffic_limit_gb
    return (
        "📊 <b>هشدار مصرف ترافیک</b>\n\n"
        f"شما <b>{used_pct:.0f}٪</b> از ترافیک خود را مصرف کرده‌اید.\n\n"
        f"📧 ایمیل: <code>{sub.email}</code>\n"
        f"📦 مصرف‌شده: <code>{used_gb:.2f} / {limit_gb} GB</code>\n\n"
        "برای خرید اشتراک جدید از منو <b>🛒 خرید کانفیگ</b> را انتخاب کنید."
    )


def _expired_text(sub: Subscription) -> str:
    """متن اعلام انقضای اشتراک."""
    return (
...
async def _check_and_warn_expiry(
    ...
    text = _expiry_warning_text(sub, days_left)
...
async def send_traffic_warning(bot: "Bot", sub: Subscription, user: User) -> None:
    ...
    if used_pct < TRAFFIC_WARN_PERCENT:
        return
    text = _traffic_warning_text(sub, used_pct)
    sent = await _send_to_user(bot, user.telegram_id, text)
    if sent:
...
async def check_expired_subscriptions(bot: "Bot") -> None:
    ...
    """
    logger.info("⏰ شروع بررسی روزانه اشتراک‌ها...")
    ...
    for sub, user in rows:
        ...
        if expiry and expiry <= now:
            async with AsyncSessionLocal() as session:
                ...
                await session.commit()
            await _send_to_user(bot, user.telegram_id, _expired_text(sub))
            expired_count += 1
        ...
        if expiry:
            ...
            for threshold in EXPIRY_WARN_DAYS:
                ...
                if days_left <= threshold:
                    sent = await _check_and_warn_expiry(bot, sub, user, days_left=threshold)
                    if sent:
                        warned_count += 1
        ...
        if sub.traffic_limit_gb > 0:
            ...
            used_pct = (sub.used_traffic_bytes / limit_bytes * 100) if limit_bytes else 0
            if used_pct >= TRAFFIC_WARN_PERCENT:
                await send_traffic_warning(bot, sub, user)
                traffic_warned += 1
