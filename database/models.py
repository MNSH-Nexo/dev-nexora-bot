...
# ──────────────────────────────────────────────
# جدول اقدامات معلق از وب‌پنل
# وقتی ادمین از وب‌پنل تراکنش کارت به کارت را تأیید می‌کند،
# پنل یک ردیف اینجا می‌نویسد و ربات هر ۱۵ ثانیه آن را پردازش می‌کند.
# ──────────────────────────────────────────────
...
class PendingAction(Base):
    ...
    id: Mapped[int] = MappedColumn(Integer, primary_key=True, autoincrement=True)
    type: Mapped[str] = MappedColumn(String(64), nullable=False, index=True)
    payload: Mapped[str] = MappedColumn(Text, nullable=False, default="{}")
    created_at: Mapped[datetime] = MappedColumn(DateTime(timezone=True), default=utcnow, nullable=False)

    def __repr__(self) -> str:
        return f"<PendingAction id={self.id} type={self.type}>"
