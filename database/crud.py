...
async def create_payment(
    ...
    init_status = "awaiting_review" if payment_method == "card" else "waiting"
    payment = Payment(
        user_id=user_id,
        order_id=order_id,
...
async def reply_to_ticket(
    ...
    """
    ثبت پاسخ به تیکت — معادل add_ticket_message برای استفاده از pending_actions.
    وقتی ادمین از وب‌پنل پاسخ می‌فرستد، pending_actions این تابع را صدا می‌زند.
    """
...
async def fetch_ticket_detail(
    ...
    """دریافت تیکت با پیام‌هایش — alias برای get_ticket."""
...
async def get_active_plans(session: AsyncSession) -> List[Plan]:
    ...
    از هر دو فرم 1 و True برای is_active پشتیبانی می‌کند —
    به دلیل اینکه پنل وب از better-sqlite3 استفاده می‌کند و مقدار ۱ ذخیره می‌کند.
    """
    from sqlalchemy import or_, literal
    result = await session.execute(
        select(Plan).where(
            or_(Plan.is_active == True, Plan.is_active == 1)
...
async def create_plan(
    ...
    plan = Plan(
        ...
        price_usdt=price_usdt, limit_ip=limit_ip,
