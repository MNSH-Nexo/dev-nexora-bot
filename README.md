...
[![License](https://img.shields.io/badge/License-Nexora--Attribution-purple)](LICENSE)
...
```bash
git clone https://github.com/MNSH-Nexo/Nexora-Bot.git /opt/nexora-bot
cd /opt/nexora-bot
...
3. اسکریپت نصب همه چیز را می‌پرسد، یا از `nexo-bot → Webhook Manager` استفاده کنید
...
اگر دامنه یا ساب‌دامنه دارید، می‌توانید HTTPS را با `nexo-bot` راه‌اندازی کنید:

```
nexo-bot → [7] SSL Manager → [1] Get new certificate
```
...
```bash
nexo-bot → Bot Control → [5] Live logs
# یا:
docker logs nexora_bot --tail 50
```
...
2. بررسی کنید webhook server فعال است: `nexo-bot → Webhook Manager → [4] Test webhook server`
...
```bash
cd /opt/nexora-bot && git pull
nexo-bot → Bot Control → [4] Rebuild image + restart
```
...
برای backup دستی: `nexo-bot → [4] Backup & Restore → [1] Full backup`
...
این پروژه تحت **Nexora Attribution License (NAL)** منتشر شده است.  
استفاده، تغییر و توزیع آزاد است، **به شرط حفظ اعتبار سازنده**.  
برای جزئیات کامل فایل [LICENSE](LICENSE) را مطالعه کنید.

---

<sub>ساخته‌شده توسط <a href="https://github.com/MNSH-Nexo">MNSH-Nexo</a></sub>
