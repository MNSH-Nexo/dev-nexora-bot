"""
handlers/uuid_import.py — دریافت اشتراک با UUID

کاربری که از خارج از ربات (دستی یا از طریق شخص دیگری) کانفیگ گرفته،
می‌تواند UUID خود را وارد کند تا ربات لینک اشتراک و کانفیگ‌هایش را بیاره.

امنیت:
  • UUID هر کلاینت کاملاً یونیک و تصادفی است (UUID v4 = 36 کاراکتر)
  • بدون UUID صحیح دسترسی ممکن نیست
...
async def msg_uuid_entry(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "🔑 <b>دریافت اشتراک با UUID</b>\n"
        "━━━━━━━━━━━━━━━\n"
        "اگر قبلاً برای شما کانفیگ ساخته شده، UUID خود را اینجا وارد کنید.\n\n"
        "UUID در فایل کانفیگ یا لینک <code>vless://</code> پس از <code>vless://</code> قرار دارد.\n"
        "مثال: <code>a1b2c3d4-1234-5678-abcd-ef0123456789</code>\n\n"
        "برای لغو: /cancel",
        parse_mode="HTML",
    )


@router.callback_query(F.data == "uuid_import")
async def cb_uuid_entry(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer()
    await state.set_state(UUIDImportStates.waiting_uuid)
    await callback.message.answer(
        "🔑 <b>دریافت اشتراک با UUID</b>\n"
        "━━━━━━━━━━━━━━━\n"
        "UUID خود را وارد کنید.\n"
        "مثال: <code>a1b2c3d4-1234-5678-abcd-ef0123456789</code>\n\n"
        "برای لغو: /cancel",
        parse_mode="HTML",
    )
...
# ──────────────────────────────────────────────
# دریافت UUID از کاربر → جستجو در پنل
# ──────────────────────────────────────────────
...
async def msg_receive_uuid(message: Message, state: FSMContext) -> None:
    ...
    # اگر کاربر کل لینک vless:// را paste کرده، UUID رو extract کن
    ...
    try:
        ...
        if not client:
            await wait_msg.edit_text(
                "❌ <b>UUID پیدا نشد</b>\n\n"
                "این UUID در پنل وجود ندارد یا غیرفعال است.\n"
                "مطمئن شوید UUID را درست وارد کرده‌اید.",
                parse_mode="HTML",
            )
            return
