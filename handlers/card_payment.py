...
Flow:
  ...
  ۲. شماره کارت + مبلغ (ریال و تومان) نمایش داده می‌شود
    ...
    for admin_id in settings.admin_ids:
        try:
            if receipt_msg.photo:
                # اگر کاربر کپشن همراه عکس فرستاده، آن را هم نمایش بده
                user_caption = receipt_msg.caption or ""
                full_caption = caption
                if user_caption:
                    full_caption += f"\n\n📝 <b>متن همراه رسید:</b>\n<code>{user_caption}</code>"
...
async def cb_card_approve(callback: CallbackQuery) -> None:
    ...
    async with AsyncSessionLocal() as session:
        ...
        # plan_id: در جدول Payment، فیلد inbound_id واقعاً plan_id ذخیره کرده
        # برای تراکنش‌های restore شده ممکن است plan دیگر در DB وجود نداشته باشد
        # در این صورت create_new_subscription با plan_id=0 اشتراک با اینباندهای فعال می‌سازد
